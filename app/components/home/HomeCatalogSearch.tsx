...
function ResultCard({ result }: { result: SearchResult }) {
  ...
  return (
    <Link
      href={result.href}
      className="group flex min-w-0 gap-3 rounded-2xl border border-black/10 bg-white/75 p-3 text-left shadow-sm backdrop-blur-md transition hover:-translate-y-0.5 hover:border-black/25 hover:bg-white dark:border-white/10 dark:bg-black/80 dark:hover:border-white/25 dark:hover:bg-zinc-950 sm:p-4"
    >
    ...
    <section aria-labelledby="home-search-title" className="pb-10 pt-5 text-foreground sm:pb-14 sm:pt-10">
      <div className="mx-auto w-full max-w-5xl text-center">
        <BlurText
          ...
          delay={90}
          stepDuration={0.28}
          className="mx-auto max-w-4xl text-3xl font-medium leading-tight tracking-[-0.035em] sm:text-5xl lg:text-6xl"
        />
        <p className="mx-auto mt-5 max-w-3xl text-sm leading-6 text-muted-foreground sm:text-base sm:leading-7">
        ...
        >
          <div className="flex h-16 min-w-0 items-center gap-3 rounded-2xl border border-black/15 bg-white/85 px-5 shadow-[0_16px_50px_rgba(15,23,42,0.08)] backdrop-blur-md focus-within:border-cyan-600/70 focus-within:ring-2 focus-within:ring-cyan-600/20 dark:border-white/20 dark:bg-black/85 dark:shadow-[0_18px_60px_rgba(0,0,0,0.28)] dark:focus-within:border-cyan-300/70 dark:focus-within:ring-cyan-300/20 sm:h-[72px]">
            {loading ? (
